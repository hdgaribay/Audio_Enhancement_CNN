...
_CONFIG_PATH = Path(__file__).resolve().parent.parent / "config.yaml" # create yaml file path
with open(_CONFIG_PATH,"r") as  _f: # open and read config file
...
def compute_stft(waveform, n_fft = _N_FFT, hop_length = _HOP, win_length = _WIN_LENGTH):
    ...
    stft_result = torch.stft(waveform_tensor,n_fft = n_fft, hop_length = hop_length, win_length = win_length, window = window, center = True, return_complex = True)
...
def compute_istft(magnitude,phase,n_fft = _N_FFT ,hop_length = _HOP, win_length = _WIN_LENGTH):
    ...
    window = _WINDOW if win_length == _WIN_LENGTH else torch.hann_window(win_length)
    istft_result = torch.istft(complex_tensor, n_fft=n_fft, hop_length=hop_length, win_length=win_length, window=window)
