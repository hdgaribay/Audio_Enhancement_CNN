...
def wav_to_bits(wav):
    wav = wav / (np.max(np.abs(wav)) + 1e-8)        # normalize to [-1, 1]
    samples_int = ((wav + 1) * 127.5).astype(np.uint8)  # to 8-bit integers
    bits = np.unpackbits(samples_int)                # each sample → 8 bits
    return bits
def qpsk_modulate(bits):
    bits = bits[:len(bits) // 2 * 2]                # make even length
...
def qpsk_demodulate(received, original_bits):
    ...
    ber            = np.mean(rx_bits[:n] != original_bits[:n])
    return ber
...
def run_simulation(config, checkpoint_path):
    ...
    for _, row in tqdm(test_df.iterrows(), total=len(test_df), desc="Simulating"):
        ...
        # align lengths
        min_len      = min(len(noisy_wav), len(enhanced_wav), len(clean_wav))
